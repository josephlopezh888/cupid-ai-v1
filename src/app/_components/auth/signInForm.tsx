...
const Login: NextPage = (e) => {
  ...
  const [loginRedirect, setLoginRedirect] = useLocalStorage<string | undefined>(
    "loginRedirect",
    undefined
  );
  ...
  const handleSubmit = async () => {
    if (!token) {
      showToast("Please complete captcha first.");
      ...
      if (response.status === 200) {
        ...
        try {
          ...
          if (!signInResponse || signInResponse.ok !== true) {
            showToast("404: Invalid credentials, Please try again.");
          } else {
            const targetPath = loginRedirect || "/dashboard";
            setLoginRedirect(undefined);
            await router.push(targetPath);
          }
        } catch (err) {
          showToast("An error occured. Please try again... " + err);
        }
      }
    } catch (error) {
      showToast('An error occurred while verifying reCAPTCHA.');
    }
  }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-black text-white">
      ...
      <div className=" xs:w-full h-screen w-full rounded-xl border-2 border-zinc-700 bg-black px-4 py-8 shadow-none sm:h-auto sm:w-8/12 sm:px-8 sm:shadow-lg md:w-7/12 lg:w-7/12 xl:w-2/6">
        ...
        <p className="text-gray-400">Login to continue using CupidAI.</p>
        ...
        <div className="flex flex-col gap-4 px-0 py-4">
          ...
          <div className="mt-6 flex w-full flex-col gap-2">
            <button onClick={handleSubmit}
              className="flex w-full flex-row items-center justify-center gap-2 rounded-[5px] bg-gradient-to-r from-[#7F04E3] to-[#FF006B] p-3   text-white transition-all duration-500 hover:from-[#7F04E3] hover:to-[#ff006a75]"
            >
              Login
            </button>
          </div>
          ...
          <div className="mt-2 flex w-full flex-row justify-center">
            ...
            <a href="/auth/signup" className="underline text-[#7F04E3]">
              Sign Up
            </a>
          </div>
        </div>
